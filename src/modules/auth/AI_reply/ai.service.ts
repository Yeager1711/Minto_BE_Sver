...
export class AI_Service {
        ...
        constructor(
            ...
            - Lưu ý: Khi khách hàng chọn số lượng khách mời, nếu hơn 20 khách. Hệ thông sẽ tính phí thêm 500đ, bắt đầu từ người thứ 21.
            -[Lý do từ khách 21 trở đi phát sinh thêm 500đ]: Với thiệp online, không giống như thiệp in ấn là tốn thêm giấy mực mà nó nằm ở hạ tầng, lưu trữ, gửi thiệp, mỗi khách 21 trở đi sẽ phát sinh thêm dung lượng lưu trử, chi phí duy trì server.
        ...
        private async getOrCreateChatSession(userId: string): Promise<ChatSession> {
                ...
                // Clear inactive sessions (e.g., > 1 hour)
                if (
                        userSession &&
        ...
        async answerAsMintoBot(userId: string, question: string): Promise<string | Templates[]> {
                // Log question with userId and timestamp
                const timestamp = new Date().toISOString(); // e.g., 2025-08-14T12:12:00.000Z
                console.log(
                ...
                try {
                        ...
                        if (wantsTemplate) {
                                ...
                                if (pickedTemplates.length > 0) {
                                        const response = `Em đã tìm thấy một vài template thiệp cưới phù hợp với sở thích của Anh/Chị: ${pickedTemplates.map((t) => t.name).join(', ')}. Anh/Chị muốn biết thêm chi tiết về mẫu nào không? 😊`;
                                        await chatSession.sendMessage(question);
                                        await chatSession.sendMessage(response);
                                        return pickedTemplates.slice(0, 3);
                                } else {
                                        const response = `
                        Không tìm thấy template nào phù hợp với sở thích Anh/Chị mô tả. 😔 Anh/Chị có thể thử mô tả chi tiết hơn (ví dụ: phong cách hiện đại, cổ điển, tối giản, hoặc cung cấp màu chủ đạo).
                    `;
                                        await chatSession.sendMessage(question);
                                        await chatSession.sendMessage(response);
                                        return this.wrapUrlsInAnchorTags(
